...
class glmGCN(object):
    ...
    def build(self):
        ...
        dense1 = tf.nn.dropout(dense1, keep_prob=keep_probability, seed=123)
        dense2 = tf.contrib.layers.fully_connected(dense1, 2048, activation_fn=tf.nn.relu)
        dense2 = tf.nn.dropout(dense2, keep_prob=keep_probability, seed=123)
        dense3 = tf.contrib.layers.fully_connected(dense2, 1024, activation_fn=tf.nn.relu)
        dense3 = tf.nn.dropout(dense3, keep_prob=keep_probability, seed=123)
        dense4 = tf.contrib.layers.fully_connected(dense3, 512, activation_fn=tf.nn.relu)
        dense4 = tf.nn.dropout(dense4, keep_prob=keep_probability, seed=123)
